...
[**ListCertsWithIDs**](CertsAPI.md#ListCertsWithIDs) | **Get** /tyk/certs/{certID} | Return one certificate or list multiple certificates in the Tyk Gateway given a comma separated list of cert IDs.
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.CertsAPI.ListCertsWithIDs(context.Background(), certID).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CertsAPI.ListCertsWithIDs``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListCertsWithIDs`: ListCertsWithIDs200Response
	fmt.Fprintf(os.Stdout, "Response from `CertsAPI.ListCertsWithIDs`: %v\n", resp)
}
```
