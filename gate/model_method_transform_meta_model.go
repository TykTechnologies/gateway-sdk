/*
Tyk Gateway API

The Tyk Gateway API is the primary means for integrating your application with the Tyk API Gateway system. This API is very small, and has no granular permissions system. It is intended to be used purely for internal automation and integration.  **Warning: Under no circumstances should outside parties be granted access to this API.**  The Tyk Gateway API is capable of:  * Managing session objects (key generation) * Managing and listing policies * Managing and listing API Definitions (only when not using the Dashboard) * Hot reloads / reloading a cluster configuration * OAuth client creation (only when not using the Dashboard)   In order to use the Gateway API, you'll need to set the `secret` parameter in your tyk.conf file.  The shared secret you set should then be sent along as a header with each Gateway API Request in order for it to be successful:  ``` x-tyk-authorization: <your-secret> ``` <br/> <b>The Tyk Gateway API is subsumed by the Tyk Dashboard API in Pro installations.</b>

API version: 4.3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gate

import (
	"encoding/json"
)

// MethodTransformMetaModel struct for MethodTransformMetaModel
type MethodTransformMetaModel struct {
	Method *string `json:"method,omitempty"`
	Path *string `json:"path,omitempty"`
	ToMethod *string `json:"to_method,omitempty"`
}

// NewMethodTransformMetaModel instantiates a new MethodTransformMetaModel object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMethodTransformMetaModel() *MethodTransformMetaModel {
	this := MethodTransformMetaModel{}
	return &this
}

// NewMethodTransformMetaModelWithDefaults instantiates a new MethodTransformMetaModel object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMethodTransformMetaModelWithDefaults() *MethodTransformMetaModel {
	this := MethodTransformMetaModel{}
	return &this
}

// GetMethod returns the Method field value if set, zero value otherwise.
func (o *MethodTransformMetaModel) GetMethod() string {
	if o == nil || o.Method == nil {
		var ret string
		return ret
	}
	return *o.Method
}

// GetMethodOk returns a tuple with the Method field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MethodTransformMetaModel) GetMethodOk() (*string, bool) {
	if o == nil || o.Method == nil {
		return nil, false
	}
	return o.Method, true
}

// HasMethod returns a boolean if a field has been set.
func (o *MethodTransformMetaModel) HasMethod() bool {
	if o != nil && o.Method != nil {
		return true
	}

	return false
}

// SetMethod gets a reference to the given string and assigns it to the Method field.
func (o *MethodTransformMetaModel) SetMethod(v string) {
	o.Method = &v
}

// GetPath returns the Path field value if set, zero value otherwise.
func (o *MethodTransformMetaModel) GetPath() string {
	if o == nil || o.Path == nil {
		var ret string
		return ret
	}
	return *o.Path
}

// GetPathOk returns a tuple with the Path field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MethodTransformMetaModel) GetPathOk() (*string, bool) {
	if o == nil || o.Path == nil {
		return nil, false
	}
	return o.Path, true
}

// HasPath returns a boolean if a field has been set.
func (o *MethodTransformMetaModel) HasPath() bool {
	if o != nil && o.Path != nil {
		return true
	}

	return false
}

// SetPath gets a reference to the given string and assigns it to the Path field.
func (o *MethodTransformMetaModel) SetPath(v string) {
	o.Path = &v
}

// GetToMethod returns the ToMethod field value if set, zero value otherwise.
func (o *MethodTransformMetaModel) GetToMethod() string {
	if o == nil || o.ToMethod == nil {
		var ret string
		return ret
	}
	return *o.ToMethod
}

// GetToMethodOk returns a tuple with the ToMethod field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MethodTransformMetaModel) GetToMethodOk() (*string, bool) {
	if o == nil || o.ToMethod == nil {
		return nil, false
	}
	return o.ToMethod, true
}

// HasToMethod returns a boolean if a field has been set.
func (o *MethodTransformMetaModel) HasToMethod() bool {
	if o != nil && o.ToMethod != nil {
		return true
	}

	return false
}

// SetToMethod gets a reference to the given string and assigns it to the ToMethod field.
func (o *MethodTransformMetaModel) SetToMethod(v string) {
	o.ToMethod = &v
}

func (o MethodTransformMetaModel) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Method != nil {
		toSerialize["method"] = o.Method
	}
	if o.Path != nil {
		toSerialize["path"] = o.Path
	}
	if o.ToMethod != nil {
		toSerialize["to_method"] = o.ToMethod
	}
	return json.Marshal(toSerialize)
}

type NullableMethodTransformMetaModel struct {
	value *MethodTransformMetaModel
	isSet bool
}

func (v NullableMethodTransformMetaModel) Get() *MethodTransformMetaModel {
	return v.value
}

func (v *NullableMethodTransformMetaModel) Set(val *MethodTransformMetaModel) {
	v.value = val
	v.isSet = true
}

func (v NullableMethodTransformMetaModel) IsSet() bool {
	return v.isSet
}

func (v *NullableMethodTransformMetaModel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMethodTransformMetaModel(val *MethodTransformMetaModel) *NullableMethodTransformMetaModel {
	return &NullableMethodTransformMetaModel{value: val, isSet: true}
}

func (v NullableMethodTransformMetaModel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMethodTransformMetaModel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


